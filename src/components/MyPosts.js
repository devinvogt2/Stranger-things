    ...
    <CenterLayout>
      ...
      {selectedPost._id ? (
        ...
        <>
          ...
          {/* if we could reactivate posts (recover from soft delete), we'd show closed posts with a reactivate option here */}
          {/* <MyPostsSection>
            <label>Closed</label>
            {!closedPosts.length ? (
              <p style={{ fontSize: "12px" }}>(no closed posts)</p>
            ) : (
              closedPosts.map((post) => (
                <PostCard
                  key={post._id}
                  post={post}
                  reactivatePost={() => reactivatePost(post)}
                />
              ))
            )}
          </MyPostsSection> */}
